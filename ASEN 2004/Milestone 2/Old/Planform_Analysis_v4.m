...
global S_vert S_fuselage S_elev wConv x_le e_c_r ...
    W_payload W_ballast b d L Re e0 C_fe h rho mu e_b e_c_t e_lambda ...
...
for i=1:length(S_ref)
    ...
    wing_loading(i),bool(i),V_H(i),V_v(i)] = ...
...
legend("Calculated Wing Loading","Threshold","Upper Limit",'Location','northwest')
title("Wing Loading versus Max Range Velocity")
hold off

figure()
hold on
plot(wing_loading,max_endur_v);
yline(12,'r');
yline(7,'r');
ylim([2,15]);
%xline(56.86848,'g');
...
legend("Calculated Wing Loading","Threshold","Upper Limit",'Location','northwest')
title("Wing Loading versus Max Endurance Velocity")
hold off
...
function [LDmax,V_range,V_endur,theta_range,R_max,endurance] = glide(h,C_D,C_L,e0,AR,rho,W,S,C_D_0)
...
for i=1:4
   [Glide_LDMax(i),Glide_Range_V(i),Glide_Endur_V(i),Glide_Theta(i),...
