...
%            the  Wing Glider and calculates              %
...
Re = 200000;                % Tempest UAS Reynold's Number [dimensionless]
AR = 16.5;                  % Tempest UAS Aspect Ratio [dimensionless]
e0 = 0.601;                 % Tempest UAS Oswald's Efficiency 
C_fe = 0.0055;              % Civil Transport Skin Friction Coefficient
S_wet = 1.87453;            % Tempest UAS Wet Planform Area [m^2]
S_ref = 0.63;               % Tempest UAS Reference Planform Area [m]
d = 0.16;                   % Tempest UAS Fuselage Diameter [m]
b = 3.22;                   % Tempest UAS Wing Span [m^2]
W = 62.78;                  % Tempest UAS weight [N]
h = 1500;                   % Tempest UAS cruise altitude [m]
rho = 1.0581;               % Air density STD ATM at Tempest_h [kg/m^3]
...
Glide_End_R = zeros(1,4);
Glide_End_V = zeros(1,4);
...
[Act_Glide_LDMax,Act_Glide_Range_V,Act_Glide_Theta] = ...
...
Results = table([Glide_LDMax';mean(Glide_LDMax);Act_Glide_LDMax],...
    [Glide_Range_V';mean(Glide_Range_V);Act_Glide_Range_V],...
    [Glide_Theta';mean(Glide_Theta);Act_Glide_Theta],...
