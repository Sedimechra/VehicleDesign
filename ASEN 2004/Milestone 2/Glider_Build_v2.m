...
% FUSELAGE DIMENSIONS
d = 3;                              % Glider Fuselage Width [m] 
d = d/conF;

L = 15;                             % Glider Fuselage Length [m]
L = L/conF;

% MAIN WING
b = 39;                                  % Glider Main Wing Span [m]
b = b/conF;

w_c_r = 4;                            % Glider Main Wing Root Chord [m]
w_c_r = w_c_r/conF;
...
x_le = 6;                            % Location of the main wing leading edge in x direction [m]
x_le = x_le/conF;

% HORIZONTAL STABILIZER
e_b = 10;                             % Glider Elevator Span [m]
e_b = e_b/conF;

e_c_r = 4;                            % Glider Elevator Root Chord [m]
e_c_r = e_c_r/conF;

e_c_t = e_c_r;                          % Glider Elevator Tip Chord  [m]
e_lambda = e_c_t./e_c_r;                % Glider Elevator Taper Ratio [dimensionless]

% VERTICAL STABILIZER
v_b = 8;                             % Glider Vertical Stabilizer Span [m]
v_b = v_b/conF;

v_c_r = 4;                            % Glider Vertical Stabilizer Root Chord [m]
v_c_r = v_c_r/conF;

v_c_t = 0.75*v_c_r;                     % Glider Vertical Stabilizer Tip Chord  [m]


v_lambda = v_c_t./v_c_r;                % Glider Vertical Stabilizer Taper Ratio [dimensionless]

% PAYLOAD
payload_h = 1.5;                      % Payload Height [m]
payload_h = payload_h/conF;
payload_w = 2.3;                     % Payload Width [m]
payload_w = payload_w/conF;
payload_l = 2.75;                     % Payload Length [m]
payload_l = payload_l/conF;

x_payload_f = 4.5;                     % The location of the payload front face 
x_payload_f = x_payload_f/conF;        %   with respect to the nose cone tip [m]
...
x_ballast_f = 0.1;                      % The location of the ballast front face 
...
%% Perform the Planform Analysis

% Define Vars Vector to pass through for analysis
vars = [w_c_r w_lambda wConv W_payload W_ballast b d L Re e0org C_fe h rho mu e_b e_c_r e_c_t e_lambda v_c_r v_c_t v_lambda v_b payload_h payload_w payload_l x_le ballast_h ballast_w ballast_l v_flow reynolds_crit x_payload_f x_ballast_f Gamma];

[max_range, max_endur, max_range_v, max_endur_v, wing_loading, V_H, V_v, B, i_t_range, i_t_endur, S_wet, S_ref, W_glider, AR, x_cg,C_D_0] = planformanalysis(vars, dataAct);
...
plot(wing_loading,max_range_v,'*b');
plot(wing_loading,max_endur_v,'*r');
yline(12,'r');
yline(7,'r');
ylim([0,20]);
%xline(56.86848,'g');
...
%
function [a, a_0, C_L,C_D_wing] = finiteWingLift(data,vars,aoa_one,aoa_two)
...
function [max_range, max_endur, max_range_v, max_endur_v, wing_loading, V_H, V_v, B, i_t1, i_t2, S_wet, S_ref, W_glider, AR, x_cg, C_D_0] = planformanalysis(vars, dataAct)
...
% Location of the vertical stab leading edge in the x direction [m]
x_v_le = L - v_c_r;
...
% respect to the main wing leading edge
...
[a_w, a_0, C_L,C_D_w] = finiteWingLift(dataAct,vars,0,6);
...
% Get the Coeff of Lift of Tail
C_L_ht1 = (0 + mean(Glide_C_L_Max)*((x_cg-x_ac_w)/c_bar))./V_H;
C_L_ht2 = (0 + mean(Glide_C_L_Pmin)*((x_cg-x_ac_w)/c_bar))./V_H;

% Calculate the Aspect Ratio of Tail
AR_t = (e_b^2)/S_elev;

% Get the 3D lift curve slope of tail
a_t1 = a_0/(1+(57.3*a_0)/(pi*mean(e0)*AR_t));
a_t2 = a_0/(1+(57.3*a_0)/(pi*mean(e0)*AR_t));

alpha_w1 = mean(Glide_C_L_Max)/a_t1;
alpha_w2 = mean(Glide_C_L_Pmin)/a_t2;
...
% Calculate the Tail Incidence Angle
i_t1 = ((-1)*(a_w)*(alpha_e1)*((h_cg - h_ac_w) - ((a_t1/a_w)*V_H*(1-d_epsilion_d_alpha))))/(V_H*a_t1);
i_t2 = ((-1)*(a_w)*(alpha_e2)*((h_cg - h_ac_w) - ((a_t2/a_w)*V_H*(1-d_epsilion_d_alpha))))/(V_H*a_t2);
