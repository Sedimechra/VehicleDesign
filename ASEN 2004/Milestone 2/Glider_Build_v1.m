...
L = 0.375;                               % Glider Fuselage Length [m]

% MAIN WING
AR = 10;
b = 1;                                  % Glider Main Wing Span [m]
w_c_r = 0.1;                         % Glider Main Wing Root Chord [m]
%w_c_t = 0.071;                         % Glider Main Wing Tip Chord  [m]        
w_c_t = w_c_r;
w_lambda = w_c_t./w_c_r;                % Glider Main Wing Taper Ratio [dimensionless]


x_le = 0.15;                        % Location of the main wing leading edge in x direction [m]
%x_le = 0.36;
...
x_payload_f = 0.15;     % The location of the payload front face 
                                        %   with respect to the nose cone tip [m]
% BALLAST
W_ballast = 4;                          % Glider Ballast Weight [N]
ballast_h = 0.03;                       % Ballast Height [m]
ballast_w = 0.03;                       % Ballast Width [m]
ballast_l = 0.03;                       % Ballast Length [m]
x_ballast_f = 0.12;          % The location of the ballast front face 
                                        %   with respect to the nose cone tip [m]
% REYNOLD'S
...
% max_range = zeros(1,Num);

vars = [w_c_r w_lambda wConv W_payload W_ballast b d L Re e0org C_fe h rho mu e_b e_c_r e_c_t e_lambda v_c_r v_c_t v_lambda v_b payload_h payload_w payload_l x_le ballast_h ballast_w ballast_l v_flow reynolds_crit x_payload_f x_ballast_f Gamma AR];
...
C_L_ht1 = (C_m_ac_w + mean(Glide_C_L_Max)*((x_cg-x_ac_w)/w_c_r))./V_H;
C_L_ht2 = (C_m_ac_w + mean(Glide_C_L_Pmin)*((x_cg-x_ac_w)/w_c_r))./V_H;
